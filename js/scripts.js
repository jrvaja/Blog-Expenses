...
(function() {
	...
	var Totals = {
		...
		init : function() {
			Totals.expenses = 0;
			Totals.setSpending();
			Totals.setDifference();
			Totals.setPostCount();
         Totals.setAverageCost();
		},
		
		setSpending : function() {
			var spending = $( '.cost', Totals.table ).map(function() {
				Totals.expenses += ~~$(this).text();
			});
			
			$( '#total_expenses' ).text( '$' + Totals.expenses );
		},
		
		setDifference : function() {
			// Over/Under
			$('#difference').text(function() {
				var budget = ~~$('#budget').text().slice(1),
					result = budget - Totals.expenses;
				
				if ( result >= 0 ) return '$' + result + ' left';
				else {
					alert('You are over budget by $' + result + ' dollars! No more spending this month!');
					return '$' + result;
				}
			});
		},
		...
		setPostCount : function() {
			$('#post_count').text(function() {
				return $( '>tbody > tr', Totals.table ).length;
			});
		},

      setAverageCost : function() {
         $('#average_cost').text(function() {
            var total_count = $('> tbody > tr', Totals.table).length;
            return '$' + Math.round(Totals.expenses / total_count);
         });
      }
	...
	var Posts = {
		...
		add : function() {
			$( '.add_post' )
			   .click(function( e ) {
			   	var add_post_button = $( this ),
			   		 overlay = Posts.createOverlay( this.href + ' #add_post_form', add_post_button );
			   		 
			   	e.preventDefault();
			   				   	
			   	// When the Add Post overlay form is submitted.					
					Posts.formSubmit(overlay, function(form, id) {
						Posts.applyTemplate(form, id);
						add_post_button.fadeIn(400); 
		...
		update : function() {
			// Edit Row click
			$( 'table a.update_row' ).live('click', function(e) {
				var update_row = this;
				e.preventDefault();
				
				var overlay = Posts.createOverlay( update_row.href + ' #update_post_form' );
				
				Posts.formSubmit(overlay, function(form) {
					var title = form.find('#title').val(),
						cost = ' ' + form.find('#cost').val(),
						date = form.find('#date').val(),
						postType = form.find('select').find( ':selected' ).val();
		...
		formSubmit : function(overlay, cb) {
			// When the Add Post overlay form is submitted.
			overlay.delegate( 'form', 'submit', function(e) {
				var form = $(this),
				...
				$.post( path, form.serialize(), function( result ) {
					...
					if ( !/^\d+$/.test(result) ) {
						form.find('p.errors')[0] || form.append('<p class="errors">Did you specify a title, date, and cost for the article?</p>'); 
						return;
					}
					overlay.fadeOut(1000);
					
					// Recalculate "At a Glance"					
					cb(form, result);
					Totals.init();
				});
				e.preventDefault();
		...
		createOverlay : function( path, button ) {
			var overlay = $('<div class="overlay"></div>');
			
			// Hide the + Button
			if ( button ) button.fadeOut(300);
			...
			overlay
				.load( path, function() {
					...
					$this
					.find( '#title' ) // no specifics. Just get first input.
						.focus();
					...
					$('<span class="close icon">X</span>')
						...
						.click(function() {
							...
							button && button.fadeIn(400);
						});
					
					
					Posts.datePicker( $('#date') );		
			})
			.hide()
		...
		datePicker: function( $elem ) {
			...
			if ( i.type === 'text' ) {
				...
				$elem.replaceWith('<input type="date" id="date" name="date" value="' + $elem.val() + '" />');
			}
		},
		...
		filtering : function() {
			...
			$( '.filter select' ).change( function() {
				var select = $( this ),
				    selected = select.find( ':selected' ).val(), // the option they choose
					 trs = $( '#expenses' ).find( 'tbody > tr' );
				...
				if ( selected === 'month' ) {
					$('#filter_month')
						...
						.change(function() {
							...
							// TODO - Fix monthly filtering
							location = 'http://www.blogexpenses.com/tuts/index/date/' + m;
						});
					return;
				}
				...
				// then find all tr's that are not that, and remove them
				trs
					.show()
					.filter('tr:not(.' + selected + ')')
					.hide();
					
			});			
		},
		
		applyTemplate : function( form, id ) {
			var template = {
				title : form.find( '#title' ).val(),
				cost : form.find( '#cost' ).val(),
				postType : form.find( 'select option:selected' ).val(),
				date : form.find('#date').val(),
				id : id
			};
			
			$( '#rowTemplate' ).tmpl( template ).appendTo( '#expenses tbody' );
		}
	}
